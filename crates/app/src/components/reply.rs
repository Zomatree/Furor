...
pub fn Reply<'a>(cx: Scope<'a, ReplyProps<'a>>) -> Element<'a> {
    ...
    cx.render(match reply.get() {
        Some(message) => {
            ...
            let (username, avatar) = get_username_avatar(channels_state, server_members_state, revolt_config, &user_state[&message.author], &message.masquerade, Some(cx.props.channel_id), api_url);
