...
pub fn Message<'a>(cx: Scope<'a, MessageProps<'a>>) -> Element<'a> {
    ...
    let (username, avatar) = get_username_avatar(channel_state, server_members, revolt_config, user, masquerade, Some(channel), api_url);
    ...
    cx.render(rsx! {
        div {
            style: "display: flex; padding: 0.125rem; margin-top: 12px; padding-inline-end: 16px; flex-direction: column; margin-left: 10px",
            ...
            div {
                ...
                div {
                    ...
                    div {
                        ...
                        time {
                            style: "color: {theme.tertiary_foreground}",
                            "{created_at}"
                        },
                        edited.is_some().then(|| rsx! {
                            span {
                                style: "font-size: 10px; color: {theme.tertiary_foreground}",
                                "(edited)"
                            }
                        })
                    },
            ...
            if *hover.get() {
                rsx! {
                    div {
                        style: "display: flex; flex-direction: row; justify-content: flex-end; position: absolute; place-self: end; background-color: {theme.primary_header}",
                        button {
