...
pub fn Channel<'a>(cx: Scope<'a, ChannelProps<'a>>) -> Element<'a> {
    ...
    cx.render(rsx! {
        div {
            style: "display: flex; flex-direction: column; width: 100%; background-color: {theme.primary_background}; flex-grow: 1; position: relative",
            div {
                style: "height: 48px; width: 100%; background-color: {theme.primary_header}; display: flex; align-items: center; z-index: 10; position: absolute; flex: 0 0 auto; padding-left: 10px; user-select: none",
                "{name}"
            }
