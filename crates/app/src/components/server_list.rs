...
pub fn ServerList(cx: Scope) -> Element {
    ...
    let (_, avatar) = get_username_avatar(channel_state, server_members_state, revolt_config, user, &None, None, api_url);
    ...
    cx.render(rsx!(div {
        ...
        server_state.values().map(|server| {
            ...
            let icon = icon.unwrap().url(&revolt_config.features.autumn.url, api_url);
            let key = id.clone();
